...
const sendReminderEmail = async (req, res) => {
  ...
  try {
    ...
    const mailOptions = {
      ...
      text: "",
      html: `
      <p style="color: red;">This is a reminder email for checking your today's tasks</p>
...
const loginUser = async (req, res) => {
  ...
  try {
    ...
    res.cookie("token", token, {
      maxAge: date.setMonth(date.getMonth() + 1),
      httpOnly: false,
    });
    res.json({
...
const updateUserTask = async (req, res) => {
  ...
  try {
    const { userTasks } = await User.findOneAndUpdate(
      {
        _id: mongoose.Types.ObjectId(userId),
        "userTasks.taskId": req.params.taskId,
      },
  ...
  try {
    ...
    const mailOptions = {
      ...
      html: `
                ...
                <a href="${process.env.CLIENT_URL}/reset-password/${userId}">${process.env.CLIENT_URL}/reset-password/${userId}</a>
            `,
    };
